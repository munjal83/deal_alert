...
class Alert(object):
    ...
    def __repr__(self):
        return "<Alert for {} on item {} for price {}>".format(self.user_email, self.item.name, self.price_limit)
    ...
    def send(self):
        return requests.post(
            ...
            data={
                ...
                "text": "We've found a deal! ({}). To navigate to the alert, visit".format(self.item.url)

            }
        )
    @classmethod
    def find_update_alert(cls, minutes_since_update=AlertConstants.ALERT_TIMEOUT):
